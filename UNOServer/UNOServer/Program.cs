...
namespace UNOServer
{
    ...
    class Program
    {
                ...
                { 
                    if (User.PlayerSocket.Available > 0) //Nếu có dữ liệu đến từ client thì server sẽ bắt đầu nhận
                    {
                        ...
                        while (User.PlayerSocket.Available > 0)
                        {
                            ...
                            if (IsBase64String(receivedata)) //nếu tin nhận được là mã hóa
                            {
                            ...
                            else
                            {
                                ...
                                AnalyzingMessage(receivedata, User); //Thông điệp được đưa vào hàm này để xử lý yêu cầu (thông điệp) từ người chơi (client) tương ứng
                            }
                        }
                    }
                }
        ...
        */
        ...
        {
            string connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=D:\C++\NEWUNOTEST\Test2\UNOServer\UNOServer\UserDatabase.mdf;Integrated Security=True;";
        ...
        {
            string connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=D:\C++\NEWUNOTEST\Test2\UNOServer\UNOServer\Database1.mdf;Integrated Security=True;";
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
        ...
        //Hàm kiểm tra dữ liệu nhận vào có phải mã hóa hay không?
        public static bool IsBase64String(string base64)
        {
            //Kiểm tra độ dài: Base64 hợp lệ phải chia hết cho 4
            ...
            try
            {
                ...
                return true; //Nếu không có lỗi, chuỗi là Base64 hợp lệ
            }
            catch
            {
        ...
        public static string CreatePlayerCards()
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                int pick = random.Next(BOBAI.CardName.Length); //Tạo biến int tên pick để lưu chỉ số trong mảng CardName ngẫu nhiên
        ...
        private static void HandleJoingame(string[] Message, PLAYER User)
        {
            ...
            foreach (var user in PLAYERLIST)
            {
                ...
                byte[] data = Encoding.UTF8.GetBytes(SendData); //Tạo mảng byte tên data để chứa thông điệp theo cấu trúc
                ...
                if (YELLUNOLIST.Contains(PLAYERLIST[HienTai - 1].ID))
                {
                    ...
                    foreach (var user in PLAYERLIST)
                    {
                        if (user.ID != PLAYERLIST[HienTai - 1].ID)
                        {
                            ...
                            byte[] data_ = Encoding.UTF8.GetBytes(SendData_);
        ...
        private static void HandleRutBai(string[] Message, PLAYER User)
        {
            ...
            byte[] data = Encoding.UTF8.GetBytes(SendData); //Tạo mảng byte tên data chứa thông điệp CardDraw
        ...
        private static void HandleSpecialDraw(string[] Message, PLAYER User)
        {
            ...
            byte[] data = Encoding.UTF8.GetBytes(cardstack);
